...
Este documento describe la arquitectura tecnica vigente del entrenador de integrales. La rama `Pre-V1.5_Refactor_Core` deja el core preparado como sistema plug-and-play: los modulos matematicos se registran por manifiesto, el core consume solo el modulo activo y la app no contiene logica matematica especifica.
...
Para agregar otro modulo, se agrega otro objeto al manifiesto. No se reescribe el bootstrap.
...
El registro ocurre en `TrigCoreRegistry.register(api)`. Si falta `moduleId`, el registro es bloqueante. Otros faltantes producen diagnosticos/warnings segun el contrato.
...
Si `validationMode` falta, se normaliza a `multiple-choice` por compatibilidad. Si se declara con un valor invalido, el diagnostico debe reportar `invalid-validationMode`.
...
Si el hook no existe, se usa fallback generico. No debe usarse `Date.now()` ni `Math.random()` dentro de firmas.

### IDs de opciones

Los IDs de opciones normales deben ser deterministas. Los fallback IDs pueden existir como defensa, pero no deben aparecer en templates activas normales. Si aparece `metadata.generatedFallbackId`, `Core.testTemplates()` debe exponer warning en `instanceWarnings`.
...
La infraestructura descrita corresponde al cierre tecnico V1.5 plug-and-play. Algunos campos internos (`modelVersion` y `generatorVersion`) pueden seguir reportando `1.4` por compatibilidad de serializacion del modelo. No deben interpretarse como autorizacion para recuperar capas legacy removidas.
