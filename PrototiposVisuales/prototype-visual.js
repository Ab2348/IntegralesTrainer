(function (root) {
  ...
  function enhanceStaticCollapsibles() {
    document.querySelectorAll(".nav-section").forEach((section, index) => {
      const trigger = section.querySelector(".nav-item .chev")
        ...
        : null;
      if (!trigger || trigger.dataset.visualAccordion === "true") {
        return;
      }
      ...
      content.hidden = !open;
      trigger.dataset.visualAccordion = "true";
    ...
    document.querySelectorAll(".right-card").forEach((card, index) => {
      ...
      if (!head || head.dataset.visualAccordion === "true") {
        return;
      }
      ...
      head.dataset.visualAccordion = "true";
      button.addEventListener("click", () => {
  ...
  const originalControlsFactory = App.createControlsPanel;
  App.createControlsPanel = function createVisualControlsPanel({
    Core,
    els,
    stateStore,
  }) {
  ...
  App.createFormulaPanel = function createVisualFormulaPanel({ Core, els }) {
    ...
    function render() {
      ...
      root.__visualFormulaPanel = { renderCurrentFamily };
    }
  ...
  App.createExerciseView = function createVisualExerciseView(args) {
    ...
    base.renderExercise = function renderExercise(
      ...
      if (exercise && exercise.family) {
        ...
        if (
          root.__visualFormulaPanel &&
          typeof root.__visualFormulaPanel.renderCurrentFamily === "function"
        ) {
          root.__visualFormulaPanel.renderCurrentFamily(exercise.family.id);
        }
      }
    };
